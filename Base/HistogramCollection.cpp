...
// CTOR1
////////////////////////////////////////////////////////////////////////////
HistogramCollection::HistogramCollection(const TString& name, int histoCapacity,
                                         LogLevel debugLevel)
  : MessageLogger(debugLevel), nHistoCapacity(histoCapacity), nHistograms(0), histograms(nullptr), isScaled(0), isSaved(0), isPlotted(0), isPrinted(0), collectionName(name), randomGenerator(new TRandom()), bOwnTheHistograms(true), scaled(1), saved(1), plotted(1), printed(1), notScaled(0), notSaved(0), notPlotted(0), notPrinted(0)
{
  if (reportDebug())
    cout << "HistogramCollection() started" << endl;
  ...
  if (reportDebug())
    cout << "HistogramCollection() completed" << endl;
}
...
////////////////////////////////////////////////////////////////////////////
void HistogramCollection::addToList(TH1* h, bool doScale, bool doSave,
                                    bool doPlot, bool doPrint, bool)
{
  if (reportInfo())
    cout << "HistogramCollection::addToList(...) Adding histo " << h->GetName() << " w/ index:" << nHistograms << " max:" << nHistoCapacity << " w/ doSave: " << doSave << " doPlot:" << doPlot << " doPrint:" << doPrint << " doScale:" << doScale << endl;
  // if (doSumw2)  h->Sumw2();
                                          ...
                                          bool sumw2)
{
  // createHistogram new 1D historgram
  if (reportDebug())
    cout << "Creating  1D histo " << name << " with " << n << " non uniform nBins:" << endl;
  TH1* h;
...
////////////////////////////////////////////////////////////////////////
TProfile* HistogramCollection::createProfile(const TString& name, int n_x,
                                             double* bins,
                                             const TString& title_x,
                                             const TString& title_y, bool,
                                             bool save, bool plot, bool print)
{
  if (reportDebug())
    cout << "Creating  1D profile " << name << " n_x:" << n_x << " non-uniform bins" << endl;
...
////////////////////////////////////////////////////////////////////////
TProfile2D* HistogramCollection::createProfile(
  const TString& name, int n_x, double min_x, double max_x, int n_y,
  double min_y, double max_y, const TString& title_x, const TString& title_y,
  const TString& title_z, bool, bool save, bool plot, bool print, bool)
...
{
  if (reportDebug())
    cout << "HistogramCollection::addHistogramsToExtList(TList *list) started." << endl;

  for (int k = 0; k < nHistograms; k++) {
    if (isSaved[k] || all)
      list->Add(histograms[k]);
  }
  /* the instance stops the histograms ownership */
  bOwnTheHistograms = false;

  if (reportDebug())
    cout << "HistogramCollection::addHistogramsToExtList(TList *list) completed." << endl;
}
...
{
  if (reportDebug())
    cout << "HistogramCollection::scale(double factor) started." << endl;
  for (int k = 0; k < nHistograms; k++) {
  ...
  if (reportDebug())
    cout << "HistogramCollection::scale(double factor) completed." << endl;
}
...
double HistogramCollection::calculateN1N1(const TH1* h_1, const TH1* h_2, TH1* h_12, double a1, double a2)
{
  if (!h_1 || !h_2 || !h_12) {
    if (reportDebug())
      cout << "-E- calculateN1N1(...) Null pointers as arguments" << endl;
    if (reportDebug())
      cout << "ABORT!!!!!" << endl;
    return -1.;
  }
  ...
  if (n3 != n1 * n2) {
    if (reportDebug())
      cout << "-E- calculateN1N1_H1H1(...) Incompatible histo dimensions" << endl;
    if (reportDebug())
...
double HistogramCollection::calculateN1N1_H1H1H2(const TH1* h_1, const TH1* h_2, TH2* h_12, double a1, double a2)
{
  if (!h_1 || !h_2 || !h_12) {
    if (reportDebug())
      cout << "-E- calculateN1N1_H1H1H2(...) Null pointers as arguments" << endl;
    if (reportDebug())
      cout << "ABORT!!!!!" << endl;
    return -1.;
  }
  ...
  if (n3x != n1 || n3y != n2) {
    if (reportDebug())
      cout << "-E- calculateN1N1_H1H1H2(...) Incompatible histo dimensions" << endl;
    if (reportDebug())
      cout << "H1: " << h_1->GetName() << " nBins:" << n1 << endl;
    if (reportDebug())
      cout << "H2: " << h_2->GetName() << " nBins:" << n2 << endl;
...
double HistogramCollection::calculateN1N1_H2H2H2(const TH2* h_1, const TH2* h_2, TH2* h_12, double a1, double a2)
{
  if (!h_1 || !h_2 || !h_12) {
    if (reportDebug())
      cout << "-E- calculateN1N1_H2H2H2(...) Null pointers as arguments" << endl;
    if (reportDebug())
      cout << "ABORT!!!!!" << endl;
    return -1.;
  }
  ...
  if (n3x != (n1x * n1y) || n3y != (n2x * n2y)) {
    if (reportDebug())
      cout << "-E- calculateN1N1_H2H2H2(...) Incompatible histo dimensions" << endl;
    if (reportDebug())
      ...
      for (int iz = 0; iz < n; ++iz) {
        ...
        switch (single) {
          ...
          case 1:
            ...
            break;
          default:
            return; // error("calculateN2N1()") << "invalid argument"<< endl; return;
        }
...
bool HistogramCollection::sameDimensions1D(const TH1* h1, const TH1* h2)
{
  ...
  if (n1x == n2x) {
    ...
    if (reportDebug())
      cout << "-ERROR- Histograms titled " << h1->GetTitle() << " and " << h2->GetTitle() << " have incompatible dimensions" << endl;
    return false;
  }
}
...
void HistogramCollection::calculateDptDpt(const TH2* spp, const TH2* spn,
                                          const TH2* snp, const TH2* snn,
                                          const TH2* avgp1, const TH2* avgp2,
                                          TH2*, TH2* dptdpt,
                                          bool ijNormalization, int nEta,
                                          int nPhi)
{
  if (!sameDimensions2D(spp, spn))
    return;
  if (!sameDimensions2D(spp, snp))
    return;
  if (!sameDimensions2D(spp, snn))
    return;
  // int nx = spp->GetNbinsX();
  // int ny = spp->GetNbinsY();

  double v1, v2, v3, v4, ev4, v5, ev5, v6, ev6, p1, p2;
  int k1, k2;
    ...
    for (int iPhi1 = 0; iPhi1 < nPhi; ++iPhi1) {
      ...
      for (int iEta2 = 0; iEta2 < nEta; ++iEta2) {
        ...
        for (int iPhi2 = 0; iPhi2 < nPhi; ++iPhi2) {
          ...
          v1 = spp->GetBinContent(k1, k2);
          v2 = spn->GetBinContent(k1, k2);
          v3 = snp->GetBinContent(k1, k2);
          v4 = snn->GetBinContent(k1, k2);
          ev4 = snn->GetBinError(k1, k2);
                                          ...
                                          bool ijNormalization, int nBins)
{
  ...
  for (int i1 = 1; i1 <= nBins; ++i1) {
    ...
    for (int i2 = 1; i2 <= nBins; ++i2) {
      ...
      v1 = spp->GetBinContent(i1, i2);
      v2 = spn->GetBinContent(i1, i2);
      v3 = snp->GetBinContent(i1, i2);
      v4 = snn->GetBinContent(i1, i2);
      ev4 = snn->GetBinError(i1, i2);
      if (v4 != 0) // && ev4/v4<0.5)
      {
        if (ijNormalization) {
          v5 = 2 * (v1 - v2 * p2 - p1 * v3 + p1 * p2 * v4);
...
void HistogramCollection::calculateSc(const TH1* spp, const TH1* n1n1, const TH1* pt1pt1, TH1* sean, bool ijNormalization)
{
  ...
  double v1, ev1, v2, v3, v4, ev4;
  for (int i1 = 1; i1 <= n1; ++i1) {
    v1 = spp->GetBinContent(i1);
    ev1 = spp->GetBinError(i1);
    v2 = n1n1->GetBinContent(i1);
    v3 = pt1pt1->GetBinContent(i1);
    if (v2 > 0) {
      if (ijNormalization)
        v4 = 2 * v1 / v2 - v3;
      else
    ...
    sean->SetBinContent(i1, v4);
    sean->SetBinError(i1, ev4);
  }
}
...
{
  if (reportDebug())
    cout << "calculateSean_H1H2H2H2(  ) Started" << endl;
  ...
  for (int i1 = 1; i1 <= n2x; ++i1) {
    for (int i2 = 1; i2 <= n2y; ++i2) {
      v1 = a1 * spp->GetBinContent(i1, i2);
      ev1 = a1 * spp->GetBinError(i1, i2);
      v2 = a2 * n1n1->GetBinContent(i1, i2);
      v3 = pt1pt1->GetBinContent(i1, i2);
      ...
      sean_D->SetBinContent(i1, i2, v4 / pt1avg / pt2avg);
      sean_D->SetBinError(i1, i2, ev4 / pt1avg / pt2avg);
      sean_T->SetBinContent(i2, i1, v4 / pt1avg / pt2avg);
      sean_T->SetBinError(i2, i1, ev4 / pt1avg / pt2avg);
    }
  }
  sean->Reset();
  ...
  delete sean_D;
  if (reportDebug())
    cout << "calculateSean_H1H2H2H2(  ) Done" << endl;
}
...
/* calculate the balance functions components associated to passed R2 */
/* two components are alway computed, for LS they will match but don't for US */
/* R2 is already reduced, projected and shifted so no symmetriztion is taken */
void HistogramCollection::calculateBfR2(const TH2* r2, const TH2* n1_1, const TH2* n1_2, TH2* bf_12, TH2* bf_21)
{
  int nbinsx = r2->GetNbinsX();
  int nbinsy = r2->GetNbinsY();
  ...
  for (int ix = 0; ix < nbinsx; ++ix) {
    for (int iy = 0; iy < nbinsy; ++iy) {
      double v = r2->GetBinContent(ix + 1, iy + 1);
      double ve = r2->GetBinError(ix + 1, iy + 1);
      ...
      bf_12->SetBinContent(ix + 1, iy + 1, v12);
      bf_12->SetBinError(ix + 1, iy + 1, v12e);
      bf_21->SetBinContent(ix + 1, iy + 1, v21);
      bf_21->SetBinError(ix + 1, iy + 1, v21e);
    }
  }
  bf_12->SetEntries(r2->GetEntries());
  bf_21->SetEntries(r2->GetEntries());
}
...
void HistogramCollection::calculateSean_H1H2H2H2(const TH1* spp, const TH2* n1n1, const TH2* pt1pt1, TH2* sean, bool ijNormalization, double a1, double a2)
{
  ...
  for (int i1 = 1; i1 <= n2x; ++i1) {
    for (int i2 = 1; i2 <= n2y; ++i2) {
      v1 = a1 * spp->GetBinContent(index);
      ev1 = a1 * spp->GetBinError(index);
      v2 = a2 * n1n1->GetBinContent(i1, i2);
      v3 = pt1pt1->GetBinContent(i1, i2);
      // v3  = 0.6*0.6;//pt1pt1->GetBinContent(15,2);    ev3 = pt1pt1->GetBinError(15,2);
      ...
      sean->SetBinContent(i1, i2, v4);
      sean->SetBinError(i1, i2, ev4);
      index++;
    }
  }
  if (reportDebug())
...
void HistogramCollection::calculateN1N1H2H2_Q3D_MCY(TH2* n1_1, TH2* n1_2,
                                                    TH3* n1n1_Q3D, double,
                                                    double)
{
...
void HistogramCollection::calculateN1N1H2H2_Q3D_MCEta(TH2* n1_1, TH2* n1_2,
                                                      TH3* n1n1_Q3D, double,
                                                      double)
{
  ...
  for (int i1 = 1; i1 <= nx; ++i1) {
    for (int i2 = 1; i2 <= ny; ++i2) {
      for (int i3 = 1; i3 <= nz; ++i3) {
        v1 = a1 * n2_Q3D->GetBinContent(i1, i2, i3);
        v2 = a2 * n1n1_Q3D->GetBinContent(i1, i2, i3);
        if (v1 > 0 && v2 > 0) {
          er1 = 0; // ev1/v1;
          er2 = 0; // ev2/v2;
          if (er1 < 0.5 && er2 < 0.5) {
...
void HistogramCollection::setHistogram(TH1* h, double v, double ev)
{
  if (h) {
    ...
    if (reportDebug())
      cout << "-E- setHistogram(TH1 * h, ...) histogram does not exist." << endl;
  }
}
...
TH1* HistogramCollection::cloneH1(const TH1* h1, const TString& histoName)
{
  if (!h1) {
    if (reportDebug())
      cout << "Could not clone histogram " << histoName << endl;
  }
  TH1* h = (TH1*)h1->Clone();
  if (!h) {
    if (reportDebug())
...
{
  if (reportDebug())
    cout << "scaleByBinWidth(TH1 * h, double scale) 1D histogram " << h->GetName()
...
void HistogramCollection::scaleByBinWidth2D(TH2* h, double scale)
{
  ...
  if (reportDebug())
    cout << "scaleByBinWidth(TH2 * h, double scale) 2D histogram " << h->GetName()
         << " will be scaled by a common scale of " << scale
...
void HistogramCollection::scaleByBinWidth(TH1* h, double scale)
{
  ...
  if (c == TProfile::Class()) {
    ...
    if (reportDebug())
      cout << "1D Profile named:" << h->GetName() << " will NOT be scaled." << endl;
  } else if (c == TH1F::Class() || c == TH1D::Class()) {
    if (reportDebug())
      cout << "1D Histo named:" << h->GetName() << " will  be scaled by binwidth and given scale " << scale << endl;
    scaleByBinWidth1D(h, scale);
  } else if (c == TH2F::Class() || c == TH2D::Class()) {
    if (reportDebug())
      cout << "2D Histo named:" << h->GetName() << " will  be scaled by binwidth and given scale " << scale << endl;
    TH2* h2 = (TH2*)h;
    scaleByBinWidth2D(h2, scale);
  } else if (c == TH3F::Class() || c == TH3D::Class()) {
    if (reportDebug())
      cout << "3D Histo named:" << h->GetName() << " will  be scaled by binwidth and given scale " << scale << endl;
    TH3* h3 = (TH3*)h;
    scaleByBinWidth3D(h3, scale);
  } else if (c == TProfile2D::Class()) {
    if (reportDebug())
      cout << "2D Profile named:" << h->GetName() << " will NOT be scaled." << endl;
  } else {
...
void HistogramCollection::sumw2All()
{
  ...
  for (int i = 0; i < nHistograms; ++i) {
    ...
    if (c == TProfile::Class()) {
      if (reportDebug())
        cout << "Profile named:" << h->GetName() << " will not be sumw2" << endl;
    } else {
      if (reportDebug())
        cout << "Histo named:" << h->GetName() << " will  be sumw2" << endl;
      if (h->GetEntries() > 0)
        h->Sumw2();
    }
  }
...
void HistogramCollection::unpack_vsXY_to_vsXVsY(const TH1* source, TH2* target)
{
  ...
  if (n != n_x * n_y) {
    if (reportDebug())
      cout << "-E- unpack_vsXY_to_vsXVsY(...)  Incompatible histogram dimensions" << endl;
    if (reportDebug())
  ...
  for (iEta = 0; iEta < nEta; iEta++) {
    for (iPhi = 0; iPhi < nPhi; iPhi++) {
      for (iDphi = 1; iDphi < 7; iDphi++) {
        iPhiA = iPhi + iDphi;
        if (iPhiA >= nPhi)
          iPhiA -= nPhi; // Dphi12 = iPhi-iPhiA < 0
        iPhiB = iPhi - iDphi;
        if (iPhiB < 0)
          iPhiB += nPhi; // Dphi12 = iPhi-iPhiB > 0
                         // Deta==0
...
void HistogramCollection::calculateR2_H1H1H1(const TH1* n2_12, const TH1* n1n1_12, TH1* r2_12, bool ijNormalization, double a1, double a2)
{
  if (!n2_12 || !n1n1_12 || !r2_12) {
    if (reportDebug())
      cout << "-E- calculateR2_H1H1H1(...) Null pointers as arguments" << endl;
    if (reportDebug())
      cout << "ABORT!!!!!" << endl;
    return;
  }
  ...
  if (n2_12_n_x != n1n1_12_n_x || n2_12_n_x != r2_12_n_x) {
    if (reportDebug())
      cout << "-E- calculateR2_H1H1H1(...) Incompatible histo dimensions" << endl;
    if (reportDebug())
  ...
  for (int i1 = 1; i1 <= n2_12_n_x; ++i1) {
    v1 = a1 * n2_12->GetBinContent(i1);
    ev1 = a1 * n2_12->GetBinError(i1);
    v2 = a2 * n1n1_12->GetBinContent(i1);
    ev2 = a2 * n1n1_12->GetBinError(i1);
    ...
    r2_12->SetBinContent(i1, v);
    r2_12->SetBinError(i1, ev);
  }
}
...
void HistogramCollection::calculateR2_H2H2H2(const TH2* n2_12, const TH2* n1n1_12, TH2* r2_12, bool ijNormalization, double a1, double a2)
{
  if (!n2_12 || !n1n1_12 || !r2_12) {
    if (reportDebug())
      cout << "-E- calculateR2_H2H2H2(...) Null pointers as arguments" << endl;
    if (reportDebug())
      cout << "ABORT!!!!!" << endl;
    return;
  }
  ...
  if (n2_12_n_x != n1n1_12_n_x || n2_12_n_x != r2_12_n_x || n2_12_n_y != n1n1_12_n_y || n2_12_n_y != r2_12_n_y) {
    if (reportDebug())
      cout << "-E- calculateR2_H2H2H2(...) Incompatible histo dimensions" << endl;
    if (reportDebug())
      cout << "H1: " << n2_12->GetName() << " n_x:" << n2_12_n_x << " n_y:" << n2_12_n_y << endl;
    if (reportDebug())
...
void HistogramCollection::calculateR2_H1H2H2(const TH1* n2_12, const TH2* n1n1_12, TH2* r2_12, bool ijNormalization, double a1, double a2)
{
  if (!n2_12 || !n1n1_12 || !r2_12) {
    if (reportDebug())
      cout << "-E- calculateR2(...) Null pointers as arguments" << endl;
    if (reportDebug())
      cout << "ABORT!!!!!" << endl;
    return;
  }
  ...
  if (n2_12_n_x != (n1n1_12_n_x * n1n1_12_n_y) || n1n1_12_n_x != r2_12_n_x || n1n1_12_n_y != r2_12_n_y) {
    if (reportDebug())
      cout << "-E- calculateR2_H1H2H2(...) Incompatible histo dimensions" << endl;
    if (reportDebug())
      cout << "H1: " << n2_12->GetName() << " n_x:" << n2_12_n_x << endl;
    if (reportDebug())
      cout << "H2: " << n1n1_12->GetName() << " n_x:" << n1n1_12_n_x << " n_y:" << n1n1_12_n_y << endl;
  ...
  for (int i_x = 1; i_x <= n1n1_12_n_x; ++i_x) {
    for (int i_y = 1; i_y <= n1n1_12_n_y; ++i_y) {
      v1 = a1 * n2_12->GetBinContent(i);
      ev1 = a1 * n2_12->GetBinError(i);
      v2 = a2 * n1n1_12->GetBinContent(i_x, i_y);
      ev2 = a2 * n1n1_12->GetBinError(i_x, i_y);
      if (v1 > 0 && v2 > 0 && ev1 / v1 < 0.5 && ev2 / v2 < 0.5) {
        if (ijNormalization) // account for the fact only half the pairs were counted
...
void HistogramCollection::calculateR2VsM(const TProfile* h1, const TProfile* h2, const TProfile* h12, TH1* r2VsM, TH1* intR2, bool sameFilter)
{
  if (!h1) {
    if (reportDebug())
      cout << "-E- calculateR2VsM( ...) h1 is a null pointer." << endl;
    return;
  }
  if (!h2) {
    if (reportDebug())
      cout << "-E- calculateR2VsM( ...) h2 is a null pointer." << endl;
    return;
  }
  if (!h12) {
    if (reportDebug())
      cout << "-E- calculateR2VsM( ...) h12 is a null pointer." << endl;
    return;
  }
  if (!r2VsM) {
    if (reportDebug())
      cout << "-E- calculateR2VsM( ...) r2VsM is a null pointer." << endl;
    return;
  }
  if (!intR2) {
    if (reportDebug())
      cout << "-E- calculateR2VsM( ...) intR2 is a null pointer." << endl;
    return;
  }
  ...
  if (n1 != n2 || n1 != n3 || n1 != n4 || n5 != 1) {
    if (reportDebug())
      cout << "-E- calculateR2VsM(...) Incompatible histogram dimensions" << endl;
    return;
  }
  ...
  for (int i = 1; i <= n1; ++i) {
    ...
    v12 = h12->GetBinContent(i);
    ev12 = h12->GetBinError(i);
    if (v1 > 0 && v2 > 0 && v12 > 0) {
      r = v12 / (v1 * v2);
...
void HistogramCollection::calculateBinCorr(const TProfile* h1, const TProfile* h2, TH1* intBinCorrVsM1, bool sameFilter)
{
  ...
  if (n1 != n2 || n3 != 1) {
    if (reportDebug())
      cout << "-E- calculateBinCorr(...) Incompatible histogram dimensions" << endl;
    return;
  }
...
void HistogramCollection::calculateAveragePt(const TH1* h1, const TH1* h2, TH1* h3)
{
  ...
  if (n1 != n2 || n2 != n3) {
    if (reportDebug())
      cout << "-E- calculateAveragePt(...) Incompatible histogram dimensions" << endl;
    if (reportDebug())
      cout << "Histo:" << h1->GetName() << " n_x:" << n1 << endl;
    if (reportDebug())
      cout << "Histo:" << h2->GetName() << " n_x:" << n2 << endl;
    if (reportDebug())
      cout << "Histo:" << h3->GetName() << " n_x:" << n3 << endl;
    return;
  }
  double v1, ev1, v2, ev2, v, ev, re1, re2;
  for (int i1 = 1; i1 <= n1; ++i1) {
    v1 = h1->GetBinContent(i1);
    ev1 = h1->GetBinError(i1);
    v2 = h2->GetBinContent(i1);
    ev2 = h2->GetBinError(i1);
    if (v1 > 0 && v2 > 0) {
      v = v1 / v2;
      re1 = ev1 / v1;
      re2 = ev2 / v2;
    ...
    h3->SetBinContent(i1, v);
    h3->SetBinError(i1, ev);
  }
}
...
void HistogramCollection::calculateAveragePtH2(const TH2* h1, const TH2* h2, TH2* h3)
{
  ...
  if (n1x != n2x || n2x != n3x || n1y != n2y || n1y != n3y) {
    if (reportDebug())
      cout << "-E- calculateAveragePtH2(...) Incompatible histogram dimensions" << endl;
    if (reportDebug())
      cout << "Histo:" << h1->GetName() << " n_x:" << n1x << endl;
    if (reportDebug())
      cout << "Histo:" << h2->GetName() << " n_x:" << n2x << endl;
    if (reportDebug())
      cout << "Histo:" << h3->GetName() << " n_x:" << n3x << endl;
    return;
  }
  double v1, ev1, v2, ev2, v, ev, re1, re2;
  for (int i_x = 1; i_x <= n1x; ++i_x) {
    for (int i_y = 1; i_y <= n1y; ++i_y) {
      v1 = h1->GetBinContent(i_x, i_y);
      ev1 = h1->GetBinError(i_x, i_y);
      v2 = h2->GetBinContent(i_x, i_y);
      ev2 = h2->GetBinError(i_x, i_y);
      if (v1 > 0 && v2 > 0) {
        v = v1 / v2;
        re1 = ev1 / v1;
        re2 = ev2 / v2;
      ...
      h3->SetBinContent(i_x, i_y, v);
      h3->SetBinError(i_x, i_y, ev);
    }
  }
}
...
void HistogramCollection::calculateAveragePtH1H2(const TH1* h1, const TH1* h2, TH2* h3)
{
  ...
  if (n1 != n2 || n2 != n3) {
    if (reportDebug())
      cout << "-E- calculateAveragePtH1H2(...) Incompatible histogram dimensions" << endl;
    if (reportDebug())
      cout << "Histo:" << h1->GetName() << " n_x:" << n1 << endl;
    if (reportDebug())
      cout << "Histo:" << h2->GetName() << " n_x:" << n2 << endl;
    if (reportDebug())
      cout << "Histo:" << h3->GetName() << " n_x:" << n3x << endl;
    if (reportDebug())
      cout << "Histo:" << h3->GetName() << " n_y:" << n3y << endl;
    return;
  }
  double v1, ev1, v2, ev2, v, ev, re1, re2;
  int i = 1;
  for (int i_x = 1; i_x <= n3x; ++i_x) {
    for (int i_y = 1; i_y <= n3y; ++i_y) {
      v1 = h1->GetBinContent(i);
      ev1 = h1->GetBinError(i);
      v2 = h2->GetBinContent(i);
      ev2 = h2->GetBinError(i);
      if (v1 > 0 && v2 > 0) {
        v = v1 / v2;
        re1 = ev1 / v1;
        re2 = ev2 / v2;
  ...
  for (int i1 = 1; i1 <= nc; i1++) {
    for (int i2 = 1; i2 <= nc; i2++) {
      for (int i3 = 1; i3 <= nc; i3++) {
        v1 = h->GetBinContent(nc + i1, nc + i2, nc + i3);
        ev1 = h->GetBinError(nc + i1, nc + i2, nc + i3);
        v2 = h->GetBinContent(nc + i1, nc + i2, nf - i3);
        ev2 = h->GetBinError(nc + i1, nc + i2, nf - i3);
        v3 = h->GetBinContent(nc + i1, nf - i2, nc + i3);
        ev3 = h->GetBinError(nc + i1, nf - i2, nc + i3);
        v4 = h->GetBinContent(nc + i1, nf - i2, nf - i3);
        ev4 = h->GetBinError(nc + i1, nf - i2, nf - i3);
        v5 = h->GetBinContent(nf - i1, nc + i2, nc + i3);
        ev5 = h->GetBinError(nf - i1, nc + i2, nc + i3);
        v6 = h->GetBinContent(nf - i1, nc + i2, nf - i3);
        ev6 = h->GetBinError(nf - i1, nc + i2, nf - i3);
        v7 = h->GetBinContent(nf - i1, nf - i2, nc + i3);
        ev7 = h->GetBinError(nf - i1, nf - i2, nc + i3);
        v8 = h->GetBinContent(nf - i1, nf - i2, nf - i3);
        ev8 = h->GetBinError(nf - i1, nf - i2, nf - i3);

        v = (v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8) / 8.0;
        ev = sqrt(ev1 * ev1 + ev2 * ev2 + ev3 * ev3 + ev4 * ev4 + ev5 * ev5 + ev6 * ev6 + ev7 * ev7 + ev8 * ev8) / 8.0;

        h->SetBinContent(nc + i1, nc + i2, nc + i3, v);
        h->SetBinError(nc + i1, nc + i2, nc + i3, ev);
        h->SetBinContent(nc + i1, nc + i2, nf - i3, v);
        h->SetBinError(nc + i1, nc + i2, nf - i3, ev);
        h->SetBinContent(nc + i1, nf - i2, nc + i3, v);
        h->SetBinError(nc + i1, nf - i2, nc + i3, ev);
        h->SetBinContent(nc + i1, nf - i2, nf - i3, v);
        h->SetBinError(nc + i1, nf - i2, nf - i3, ev);
        h->SetBinContent(nf - i1, nc + i2, nc + i3, v);
        h->SetBinError(nf - i1, nc + i2, nc + i3, ev);
        h->SetBinContent(nf - i1, nc + i2, nf - i3, v);
        h->SetBinError(nf - i1, nc + i2, nf - i3, ev);
        h->SetBinContent(nf - i1, nf - i2, nc + i3, v);
        h->SetBinError(nf - i1, nf - i2, nc + i3, ev);
        h->SetBinContent(nf - i1, nf - i2, nf - i3, v);
        h->SetBinError(nf - i1, nf - i2, nf - i3, ev);
      }
    }
  }
}
...
void HistogramCollection::reduce_n2xEtaPhi_n2DetaDphi(const TH2* source, TH2* target, int nEtaBins, int nPhiBins, Option_t* opt)
{
  ...
  for (dEta = 0; dEta < 2 * nEtaBins - 1; ++dEta) {
    for (dPhi = 0; dPhi < nPhiBins; ++dPhi) {
      ...
      if (v2 <= 0)
        if (reportDebug())
          cout << "HistogramCollection::reduce_n2xEtaPhi_n2DetaDphi() Elements of denominator are negative." << endl;
      target->SetBinContent(dEta + 1, dPhi + 1, v1 / v2);
      target->SetBinError(dEta + 1, dPhi + 1, sqrt(ev1) / v2);
    }
  }
...
void HistogramCollection::project_n2XYXY_n2XX(const TH2* source, TH2* target, int nXBins, int nYBins)
{
  ...
  /* sanity checks */
  if ((source->GetNbinsX() != nXBins * nYBins) or (source->GetNbinsY() != nXBins * nYBins) or (target->GetNbinsX() != nXBins) or (target->GetNbinsY() != nXBins)) {
    cout << "-Fatal- HistogramCollection::project_n2XYXY_n2XX: "
         << "Inconsistent indexes for histogram " << source->GetName() << endl;
    return;
  }
...
void HistogramCollection::project_n2XYXY_n2YY(const TH2* source, TH2* target, int nXBins, int nYBins)
{
  ...
  /* sanity checks */
  if ((source->GetNbinsX() != nXBins * nYBins) or (source->GetNbinsY() != nXBins * nYBins) or (target->GetNbinsX() != nYBins) or (target->GetNbinsY() != nYBins)) {
    cout << "-Fatal- HistogramCollection::project_n2XYXY_n2YY: "
         << "Inconsistent indexes for histogram " << source->GetName() << endl;
    return;
  }
...
int HistogramCollection::index2(int i1, int i2)
{
  ...
  if (v < 0)
    cout << "<E> HistogramCollection::index2(int i1, int i2) Combination i1:" << i1 << " i2:" << i2 << " is not allowed." << endl;
  return v;
}
...
int HistogramCollection::index3(int i1, int i2, int i3)
{
  ...
  if (v < 0)
    cout << "<E> HistogramCollection::index3(int i1, int i2, int i3) Combination i1:" << i1 << " i2:" << i2 << " i3:" << i3 << " is not allowed." << endl;
  return v;
}
...
int HistogramCollection::index4(int i1, int i2, int i3, int i4)
{
  ...
  if (v < 0)
    cout << "<E> HistogramCollection::index3(int i1, int i2, int i3, int i4) Combination i1:" << i1 << " i2:" << i2 << " i3:" << i3 << " i4:" << i4 << " is not allowed." << endl;
  return v;
}
...
void HistogramCollection::calculateF2R2(TH1* h_f1_1, TH1* h_f1_2, TH1* h_f2_12, TH1* h_F2_12, TH1* h_R2_12)
{
  ...
  for (int iBin = 1; iBin <= nBins; iBin++) {
    ...
    f1_2 = h_f1_2->GetBinContent(iBin);
    ef1_2 = h_f1_2->GetBinError(iBin);
    f2_12 = h_f2_12->GetBinContent(iBin);
    ef2_12 = h_f2_12->GetBinError(iBin);
...
void HistogramCollection::calculateNudyn(TH1* h_R2_11, TH1* h_R2_12, TH1* h_R2_22, TH1* h_nudyn_12)
{
  ...
  for (int iBin = 1; iBin <= nBins; iBin++) {
    R2_11 = h_R2_11->GetBinContent(iBin);
    eR2_11 = h_R2_11->GetBinError(iBin);
    R2_12 = h_R2_12->GetBinContent(iBin);
    eR2_12 = h_R2_12->GetBinError(iBin);
    R2_22 = h_R2_22->GetBinContent(iBin);
    eR2_22 = h_R2_22->GetBinError(iBin);
                                        ...
                                        TH1* h_F3_123, TH1* h_R3_123)
{
  ...
  for (int iBin = 1; iBin <= nBins; iBin++) {
    ...
    f1_2 = h_f1_2->GetBinContent(iBin);
    ef1_2 = h_f1_2->GetBinError(iBin);
    f1_3 = h_f1_3->GetBinContent(iBin);
    ef1_3 = h_f1_2->GetBinError(iBin);
    f2_12 = h_f2_12->GetBinContent(iBin);
    ef2_12 = h_f2_12->GetBinError(iBin);
    f2_13 = h_f2_13->GetBinContent(iBin);
    ef2_13 = h_f2_13->GetBinError(iBin);
    f2_23 = h_f2_23->GetBinContent(iBin);
    ef2_23 = h_f2_23->GetBinError(iBin);
    f3_123 = h_f3_123->GetBinContent(iBin);
    ef3_123 = h_f3_123->GetBinError(iBin);
                                        ...
                                        TH1* h_F4_1234, TH1* h_R4_1234)
{
  ...
  for (int iBin = 1; iBin <= nBins; iBin++) {
    ...
    ef1_2 = h_f1_2->GetBinError(iBin);
    f1_3 = h_f1_3->GetBinContent(iBin);
    ef1_3 = h_f1_3->GetBinError(iBin);
    f1_4 = h_f1_4->GetBinContent(iBin);
    ef1_4 = h_f1_4->GetBinError(iBin);
    f2_12 = h_f2_12->GetBinContent(iBin);
    ef2_12 = h_f2_12->GetBinError(iBin);
    f2_13 = h_f2_13->GetBinContent(iBin);
    ef2_13 = h_f2_13->GetBinError(iBin);
    f2_14 = h_f2_14->GetBinContent(iBin);
    ef2_14 = h_f2_14->GetBinError(iBin);
    f2_23 = h_f2_23->GetBinContent(iBin);
    ef2_23 = h_f2_23->GetBinError(iBin);
    f2_24 = h_f2_24->GetBinContent(iBin);
    ef2_24 = h_f2_24->GetBinError(iBin);
    f2_34 = h_f2_34->GetBinContent(iBin);
    ef2_34 = h_f2_34->GetBinError(iBin);
    f3_123 = h_f3_123->GetBinContent(iBin);
    ef3_123 = h_f3_123->GetBinError(iBin);
    f3_124 = h_f3_124->GetBinContent(iBin);
    ef3_124 = h_f3_124->GetBinError(iBin);
    f3_134 = h_f3_134->GetBinContent(iBin);
    ef3_134 = h_f3_134->GetBinError(iBin);
    f3_234 = h_f3_234->GetBinContent(iBin);
    ef3_234 = h_f3_234->GetBinError(iBin);
    f4_1234 = h_f4_1234->GetBinContent(iBin);
    ef4_1234 = h_f4_1234->GetBinError(iBin);
                                        ...
                                        double& F4_1234, double& eF4_1234, double& R4_1234, double& eR4_1234)
{
  ...
  if (f1_1 < 1E-20 || f1_2 < 1E-20 || f1_3 < 1E-20 || f1_4 < 1E-20) {
    ...
    F4_1234 = f4_1234 - f3_123 * f1_4 - f3_124 * f1_3 - f3_134 * f1_2 - f3_234 * f1_1 - f2_12 * f2_34 - f2_13 * f2_24 - f2_14 * f2_23 + 4.0 * (f2_12 * f1_3 * f1_4 + f2_13 * f1_2 * f1_4 + f2_14 * f1_2 * f1_3) - 6.0 * f1_1 * f1_2 * f1_3 * f1_4;
    ref1_1 = ef1_1 / f1_1;
    ref1_2 = ef1_2 / f1_2;
    ref1_3 = ef1_3 / f1_3;
    ref2_12 = ef2_12 / f2_12;
    ...
    eF4_1234 = ef4_1234 * ef4_1234 + f3_123 * f3_123 * f1_4 * f1_4 * (ref3_123 * ref3_123 + ef1_4 * ef1_4) + f3_124 * f3_124 * f1_3 * f1_3 * (ref3_124 * ref3_124 + ref1_3 * ref1_3) + f3_134 * f3_134 * f1_2 * f1_2 * (ref3_134 * ref3_134 + ref1_2 * ref1_2) + f3_234 * f3_234 * f1_1 * f1_1 * (ref3_234 * ref3_234 + ref1_1 * ref1_1) + 16.0 * f2_12 * f2_12 * f2_34 * f2_34 * (ref2_12 * ref2_12 + ef2_34 * ef2_34) + 16.0 * f2_13 * f2_13 * f2_24 * f2_24 * (ref2_13 * ref2_13 + ef2_24 * ef2_24) + 16.0 * f2_14 * f2_14 * f2_23 * f2_23 * (ref2_14 * ref2_14 + ef2_23 * ef2_23);
